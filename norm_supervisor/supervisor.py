...
class Supervisor:
    ...
    def __init__(
        ...
        :param kl_delta: maximum KL-divergence for the supervisory policy.
    ...
    def _get_safe_policy(self) -> np.ndarray:
        ...
        safe_policy = torch.clamp(safe_policy, min=1e-6)  # Avoid zero probabilities
        if self.verbose:
    ...
    def _update_policy(self, model_policy: np.ndarray, safe_policy: np.ndarray) -> np.ndarray:
        """Estimates the safest policy for which the KL-divergence is less than the threshold.
        ...
        for _ in range(self.max_iters):
            mid = (low + high) / 2
            updated_policy = model_policy ** (1 - mid) * safe_policy ** mid
            updated_policy = updated_policy / sum(updated_policy)
            kl_value = entropy(model_policy, updated_policy, base=2, nan_policy='raise')

            if abs(kl_value - self.kl_delta) < self.tol:
                break  # Found a good estimate
            if kl_value > self.kl_delta:
        ...
        updated_policy = model_policy ** (1 - best_estimate) * safe_policy ** best_estimate
        updated_policy = updated_policy / sum(updated_policy)
        if self.verbose:
            print(f"Updated Policy KL-Divergence: {entropy(model_policy, updated_policy, base=2, nan_policy='raise')}")
